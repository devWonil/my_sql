...
-- 결과값이 소수 1번째 자리에서 반올림하세요. 여기서 백만명은 숫자 0이 6개 1000000 입니다. 
...
select *
from 국가
where 지역 in ((select 지역 from 국가 where 이름 = '영국') , (select 지역 from 국가 where 이름 = '한국'));

-- 문제 14 인구가 스페인보는 많고 독일보다 적은 국가의 이름을 검색 하라
select 이름
from 국가
where 인구 > (select 인구 from 국가 where 이름 = '스페인') and 인구 < (select 인구 from 국가 where 이름 = '독일');